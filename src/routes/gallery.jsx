...
const Gallery = () => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-[30px] pb-[30px]">
        {loading && <p className="text-center">Loading...</p>}
        {error && <p className="text-center text-red-500">Failed to fetch data.</p>}
        {Array.isArray(data) && data.length === 0 ? (
          ...
          data.map((item, index) => (
            <div key={index} className="gallery-item flex items-center justify-center overflow-hidden max-w-sm mx-auto h-auto transform hover:transition duration-500 hover:scale-110 delay-100 ease-in-out hover:delay-150 hover:z-50 cursor-pointer box-border rounded-[20px]">
              {item.type === 'image' && (
                <Zoom>
                  <img src={item.src} alt={item.alt} className="object-cover w-[400px] h-[250px] box-border rounded-[20px] " />
                </Zoom>
              )}
              {item.type === 'video' &&  (
                <ReactPlayer
